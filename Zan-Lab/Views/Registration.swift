...
struct Registration: View {
    ...
    var body: some View {
        ScrollView {
                                ...
                                SecureField("Придумайте пароль", text: $password)
                                    .onReceive(Just(password)) { newValue in
                                        let filtered = newValue.filter { char in
                                            char.isNumber || "A"..."Z" ~= char || "a"..."z" ~= char || "~`!@#$%^&*()-_=+[]{}\\|<>;:'\",.?".contains(char)
                                        }
            ...
            VStack(alignment: .leading, spacing: 24) {
                if mainVM.registerPending {
                    ...
                    HStack(alignment: .center, spacing: 12){
                        ZStack(alignment: .leading) {
                            ...
                            SecureField("Введите код из SMS", text: $otpCode)
                                .keyboardType(.numberPad)
                                .onChange(of: otpCode, perform: { [oldValue = otpCode] newValue in
                                    // 4 digits only
                                    if newValue.count > 4 {
                                        self.otpCode = oldValue
                                    }
                                })
                        }
                        .padding(.vertical,  10)
                ...
                Button(action: {
                    ...
                    if !isPasswordValid(password: self.password){
                    ...
                    Text(mainVM.registerPending ? "Зарегистрироваться" : "Получить SMS код")
                        .font(.title2)
        ...
        .navigationDestination(isPresented: $mainVM.loginPending) { Login(path: $path) }
    }
}
